...
import type { ColumnDefinition, DetailsFieldRenderer } from '../components/grid/GridTable';
...
const vulnerabilityColumns: ColumnDefinition<any>[] = [
  ...
  { header: 'Selector', accessor: 'selector', sortable: true, filterable: true },
...
const vulnerabilityRenderers: DetailsFieldRenderer<any>[] = [
  {
    field: 'severity',
    render: (value) => <SeverityBadge severity={String(value)}>{String(value)}</SeverityBadge>
  },
  {
    field: 'screenshot',
...
const SeverityBadge = styled.span<{ severity: string }>`
  ...
  background-color: ${props =>
    props.severity === 'Critical' ? '#ffebee' :
    props.severity === 'Major' ? '#fff3e0' :
    '#e8f5e9'
  };
  color: ${props =>
    props.severity === 'Critical' ? '#c62828' :
    props.severity === 'Major' ? '#ef6c00' :
    '#2e7d32'
  };
`;
